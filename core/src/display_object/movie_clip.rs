...
impl<'gc> MovieClip<'gc> {
    ...
    pub fn has_unload_handler(self) -> bool {
        self.clip_actions()
            .iter()
            // TODO Should this be declared_events, or real_events?
            .any(|handler| handler.declared_events.contains(ClipEventFlag::UNLOAD))
    }
...
pub struct ClipEventHandler {
    ...
    /// The events declared on this handler.
    ///
    /// This can be different from [`ClipEventHandler::effective_events`] if
    /// events are filtered by allEventFlags.
    declared_events: ClipEventFlag,
