...
impl SwfMovie {
    ...
    pub fn empty(swf_version: u8) -> Self {
        Self {
            ...
            compressed_len: 0,
            is_movie: false,
            // TODO what security sandbox to use here?
            sandbox_type: SandboxType::LocalWithFile,
        }
    }
    ...
    pub fn from_data(
        ...
        let sandbox_type = SandboxType::detect(url.as_str(), &swf_buf.header);
        let mut movie = Self {
    ...
    pub fn from_loaded_image(url: String, length: usize) -> Self {
        ...
        let sandbox_type = SandboxType::detect(url.as_str(), &header);
        let mut movie = Self {
            header,
            data: vec![],
            url,
