...
impl FilePicker {
    ...
    pub async fn pick_ruffle_directory(&self, dir: Option<PathBuf>) -> Option<PlayingContent> {
        let locale = &self.data.preferences.language();
        let mut dialog =
        ...
        let directory: PathBuf = self.show_dialog(dialog, |d| d.pick_folder())?.await?.into();

        // Try opening a bundle...
        match Bundle::from_path(&directory) {
            Ok(bundle) => {
                let url = bundle.information().url.clone();
                return Some(PlayingContent::Bundle(url, Box::new(bundle)));
            }
            Err(err) => {
            ...
            self.data
                ...
                .send_event(RuffleEvent::OpenDialog(DialogDescriptor::PickPath(
                    PickPathDialogConfiguration::new(
                        notifier,
                        directory.clone(),
                        LocalizableText::LocalizedText("select-root-movie-title"),
                        Some(LocalizableText::LocalizedText(
                            "select-root-movie-message",
                        )),
                        LocalizableText::LocalizedText("select-root-movie-pick"),
                    ),
                )));
